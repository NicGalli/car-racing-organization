...
class CircuitRepositoryTest {
	...
	void test1() {
		Circuit circuit = new Circuit(null, "test", 1000L);
		Circuit saved = repository.save(circuit);
		Collection<Circuit> circuits = repository.findAll();
		assertThat(circuits).containsExactly(saved);
	}
	...
	void test2() {
		Circuit circuit = new Circuit(null, "test", 1000L);
		Circuit saved = entityManager.persistFlushFind(circuit);
		Collection<Circuit> circuits = repository.findAll();
		assertThat(circuits).containsExactly(saved);
	}
}
